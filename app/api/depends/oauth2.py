...
def create_token(data: dict, token_type: str):
    ...
    if token_type == "access":
        expire = datetime.now() + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRES_IN_MINUTES)
    else:
        expire = datetime.now() + timedelta(days=settings.REFRESH_TOKEN_EXPIRES_IN_DAYS)
...
def verify_token(credentials: HTTPAuthorizationCredentials, db: Session):
    ...
    user = crud.branch_account.get(db=db, entry_id=uid)
    if user is None:
        raise HTTPException(
...
def get_current_user(
    ...
    user_id = token['uid']
    user = crud.branch_account.get_branch_account_by_id(db=db, id=user_id)
    return user
